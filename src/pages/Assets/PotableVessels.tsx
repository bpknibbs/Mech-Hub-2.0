...
export default function PotableVessels() {
  ...
  useEffect(() => {
    const fetchVessels = async () => {
      const { data, error } = await supabase
        .from('assets')
        .select('*')
        .eq('asset_type', 'Potable Vessel');

      if (error) {
        console.error('Error fetching potable vessels:', error.message);
      } else {
        setVessels(data);
      }
      setLoading(false);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Potable Vessels</h1>
          <p className="mt-2 text-gray-600">
            Monitor potable water storage and pressure vessels
          </p>
        </div>
        <Button>
          ...
          <Card key={vessel.id} className="p-6 hover:shadow-lg transition-all duration-200" hover>
            ...
            <div className="space-y-2">
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className="text-gray-900">{vessel.workingPressure}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-600">Current Pressure:</span>
                <span className="text-gray-900">{vessel.currentPressure}</span>
              </div>
              <div className="flex justify-between text-sm">

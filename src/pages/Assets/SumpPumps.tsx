  ...
  useEffect(() => {
    const fetchPumps = async () => {
      const { data, error } = await supabase
        .from('assets')
        .select('*')
        .eq('asset_type', 'Sump Pump');

      if (error) {
        console.error('Error fetching sump pumps:', error.message);
      } else {
        setPumps(data);
      }
      setLoading(false);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Sump Pumps</h1>
          <p className="mt-2 text-gray-600">
            Monitor drainage and sump pump systems for water management
          </p>
        </div>
        <Button>
          ...
          <Card key={pump.id} className="p-6 hover:shadow-lg transition-all duration-200" hover>
            ...
            <div className="space-y-2 mb-4">
              <div className="flex justify-between text-sm">
                ...
                <span className="text-gray-900">{pump.capacity}</span>
              </div>
              <div className="flex justify-between text-sm">
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className={`${
                  pump.waterLevel === 'Normal' ? 'text-green-600' :
                  pump.waterLevel === 'Low' ? 'text-blue-600' : 'text-red-600'
                }`}>
                  {pump.waterLevel}
                </span>
              </div>
              <div className="flex justify-between text-sm">
                ...
                <span className="text-gray-900">{pump.runHours}h</span>
              </div>
            </div>

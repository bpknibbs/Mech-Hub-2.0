...
export default function FreshWaterBoosterPumps() {
  ...
  useEffect(() => {
    const fetchPumps = async () => {
      const { data, error } = await supabase
        .from('assets')
        .select('*')
        .eq('asset_type', 'Fresh Water Booster Pump');

      if (error) {
        console.error('Error fetching fresh water booster pumps:', error.message);
      } else {
        setPumps(data);
      }
      setLoading(false);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Fresh Water Booster Pumps</h1>
          <p className="mt-2 text-gray-600">
            Monitor and maintain fresh water booster pump systems
          </p>
        </div>
        <Button>
          ...
          <Card key={pump.id} className="p-6 hover:shadow-lg transition-all duration-200" hover>
            <div className="flex items-start justify-between mb-4">
              ...
              <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                ...
                pump.status === 'Standby' ? 'bg-yellow-100 text-yellow-800' :
            ...
            <div className="space-y-2 mb-4">
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className="text-gray-900">{pump.pressure}</span>
              </div>
              <div className="flex justify-between text-sm">
